...
func (cfg *Config) SetFromURL(in *url.URL) error {
	...
	if val := in.Query().Get(events.UseFIPSEndpoint); val != "" {
		useFips, err := strconv.ParseBool(val)
		if err != nil {
			return trace.BadParameter(boolErrorTemplate, in.String(), events.UseFIPSEndpoint, val)
		}
		if useFips {
			cfg.UseFIPSEndpoint = types.ClusterAuditConfigSpecV2_FIPSEnabled
		} else {
			cfg.UseFIPSEndpoint = types.ClusterAuditConfigSpecV2_FIPSDisabled
		}
	}
